...
// TODO: DANGER!!!
WeightMap::weight_t *WeightMap::getWeights() const {
	weight_t *const buff = new weight_t[this->width * this->height];	// vestigial from python
...
WeightMap::weight_t WeightMap::getMaxWeightInMap() const {
...
WeightMap::weight_t WeightMap::getWeight(mapsize_t x, mapsize_t y) const {
	if (!isValidPoint(x, y)) {
...
WeightMap::path_t WeightMap::getPath(mapsize_t srcX, mapsize_t srcY, mapsize_t dstX, mapsize_t dstY, weight_t turn_cost) {
...
// #ifdef HAVE_OPENCV
	...
	for (int x = 0; x < data_w; x++) {
		int map_x = std::floor(((x * data_res) + origin_x) / self_res);
		if (map_x < 0 || map_x >= width)
			continue;
		for (int y = 0; y < data_h; y++) {
			int map_y = std::floor(((y * data_res) + origin_y) / self_res);
			if (map_y < 0 || map_y >= height)
				continue;
...
WeightMap::fweight_t WeightMap::get_linear_cost(Node &a, Node &b) {
#if 1
	fweight_t sum = 0.0;
...
WeightMap::fweight_t WeightMap::distance(fweight_t x1, fweight_t y1, fweight_t x2, fweight_t y2) {
