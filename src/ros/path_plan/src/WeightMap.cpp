...
fweight_t WeightMap::get_linear_cost(Node &a, Node &b)
{
    ...
    if (b.x - a.x == b.y - a.y || b.x - a.x == a.y - b.y)
    {                                                                // line of slope 1 or -1
    ...
    if (m < 1.0)
    { // dy < dx, so incrementing x by 1 safely included all y values
        const mapsize_t x1 = std::min(a.x, b.x);
        ...
        {
            if (err + m >= 0.5f)
            {                                     // The line crosses into new vertical tile
    ...
    {                 // dy > dx, so incrementing y by 1 safely included all x values
        ...
        {
            if (err + m >= 0.5f)
            {                                     // The line crosses into new horizontal tile
    ...
    while (true)
    {
        ...
        if (diag >= 2)
        { // Add them up as integers for speed over float operations
